...
class Package
{
    ...
    {
	\Log::error("getting password...");
	\Log::error($this->manifest);
    ...
    protected function setManifestItems()
    {
	    ...
	    \Log::error($m);
            ...
            if (count($this->getServices()) == 0) {
                throw new InternalServerErrorException('N0 items found in package manifest.');
            }
        }
    }
    ...
    {
        try {
            \Log::error("In save zip file!");
            $this->zip->close();

            if ($this->isSecured()) {
		\Log::error("in vuln branch");
                $password = $this->getPassword();
		\Log::error("used password is: " . $password);
                $tmpDir = rtrim(sys_get_temp_dir(), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;
